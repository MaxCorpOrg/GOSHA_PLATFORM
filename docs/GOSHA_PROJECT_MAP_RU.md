...
- Быстрый резерв:
  - `EdgeTTS`
- Аудиоконтракт `gosha-v1` для совместимого `WebSocket`:
  - `Opus`
  - моно
  - `16000 Hz`
  - кадр `60 ms`
- Важно: это не то же самое, что внутренняя частота генерации `SileroTTS`.
  Для текущего `SileroTTS v5_5_ru` оставлена поддержанная частота `24000 Hz`, а серверный контракт устройства задаётся отдельно через `SELFHOST_XIAOZHI_AUDIO_SAMPLE_RATE`.
...
- Приёмка stacked Draft PR платформы:
  - `<LOCAL_WORKSPACE>/docs/GOSHA_STACKED_PR_ACCEPTANCE_RU.md`
