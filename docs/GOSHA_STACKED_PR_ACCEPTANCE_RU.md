...
- Не записывать в git адреса действующих личных/продуктовых endpoint, токены, пароли, коды подключения/активации, SSID, MAC, IP, аппаратный `device_id`, transcript, prompt или raw audio.

Публичные ссылки на официальную документацию и исходники, а также явно
обозначенные synthetic/localhost примеры допустимы. Личные страницы настройки
агента и ссылки на закрытый workspace заменяются маркерами независимо от
того, содержат ли они пароль или token.
...
- Все отслеживаемые документы, включая исторические checkpoint, проверены на адреса действующих личных/продуктовых endpoint, owner-only пути, пароли, токены, коды подключения/активации, SSID, MAC и аппаратные идентификаторы; значения заменены переносимыми маркерами.
