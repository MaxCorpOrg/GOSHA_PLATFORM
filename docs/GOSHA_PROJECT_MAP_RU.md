...
- Контур проверки кода для разработчика:
  - в репозитории уже есть внешний ручной сервис проверки:
    - `/home/max/GOSHA_PLATFORM/oauth_reviewer`
  - его runbook лежит в:
    - `/home/max/GOSHA_PLATFORM/docs/GOSHA_OAUTH_REVIEWER_RUNBOOK_RU.md`
  - он даёт вход через `GitHub OAuth` и вызывает OpenAI на серверной стороне, а не через secrets репозитория.
- Прошивка:
