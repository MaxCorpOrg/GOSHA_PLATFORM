...
## Аудиоконтракт совместимого `WebSocket`

- Для текущего профиля `gosha-v1` серверный `hello` должен отдавать `Opus`, моно, `16000 Hz`, кадр `60 ms`.
- Настройка хранится в `selfhost-backend.env` как `SELFHOST_XIAOZHI_AUDIO_SAMPLE_RATE`, значение по умолчанию `16000`.
- Не смешивай её с `SELFHOST_XIAOZHI_SILERO_SAMPLE_RATE`: для `SileroTTS v5_5_ru` внутренняя частота генерации остаётся `24000`, потому что модель поддерживает `8000/24000/48000`.
