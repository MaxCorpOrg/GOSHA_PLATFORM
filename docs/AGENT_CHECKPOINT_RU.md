...
- Для мобильного контура ближайший незакрытый шаг теперь практический:
  - локальный портал робота в режиме точки доступа пока не дал рабочую страницу
  - телефон уже в сети `GOSHA-A-1BE1`, но `192.168.4.1` показал пустой чёрный экран
  - значит следующий разбор нужен уже на стороне прошивки и локального портала, а не в пакете подключения
...
4. Проверить новый мобильный контур:
   - подтверждённые шаги:
     - установка `GOSHA_MOBILE`
     - ввод кода подключения
     - поиск `GOSHA-*`
     - перевод телефона в `GOSHA-A-1BE1`
   - незакрытые шаги:
     - живой радиотест переходного `Xiaozhi-*`
     - рабочее открытие `192.168.4.1`
     - переход в главное меню после настройки робота
5. Потом переходить к второй очереди:
