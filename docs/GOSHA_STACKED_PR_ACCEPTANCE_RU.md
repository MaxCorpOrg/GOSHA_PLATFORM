...
3. Проверить, что рабочая страница не смешивает блоки:
   ...
   - след устройства.
4. Убедиться, что операторский интерфейс не показывает токены, прямые OTA-URL и voice WebSocket URL. Диагностические `device_id`, `board_ip` и `remote_addr` в блоке «След устройства» соответствуют текущей продуктовой карте и доступны оператору в его защищённой сессии. Не переносить эти значения в runtime-events, общедоступные отчёты или Git. Эта строка не вводит новую политику скрытия диагностического следа устройства.
